...
class Ensemble_orderFC(nn.Module):
    def __init__(self, in_features, out_features, num_models, first_layer=False,
                 bias=True, constant_init=False, p=0.5, random_sign_init=False, layer_W = False):
    ...
    def forward(self, x):
        ...
        if self.num_models>1:
            ...
            result = self.fc(x) #self.fc(x*alpha)*gamma#
        else:
            result = None
    ...
    def forward(self, x):
        ...
        if self.train_gamma:
            ...
            if self.num_models > 1:
                ...
                result = self.conv(x) #self.conv(x*alpha)*gamma #
            else:
                result = None
            #import time
